# Working agreement — EndpointValidator
...
**Rust workspace** (`EndpointValidator`).
...
**Always paste the full PR URL** (`https://github.com/pathscale/EndpointValidator/pull/<n>`), not just the number, so it's
clickable.
...
uncomment the rule below.
    ...
    ```bash
    gh pr view <number> --repo pathscale/EndpointValidator --json statusCheckRollup
    ```
